 ...
 *
 * Engine: electron-updater. It reads the NSIS `latest.yml`
 * metadata uploaded to GitHub Releases by electron-builder.
 *
 ...
 *   - 'available':   a newer version exists, user can download it.
 *   - 'downloading': update bytes are flowing.
 ...
 * return without touching `autoUpdater`.
 */
export function initAutoUpdater(): void {
  ...
  // Point electron-updater at GitHub Releases. electron-builder uploads
  // `latest.yml`, the NSIS installer, and blockmap assets for this feed.
  try {
  ...
  autoUpdater.logger = {
    debug: (message: string) => logInfo('updater', message),
    info: (message: string) => logInfo('updater', message),
    warn: (message: string) => logWarn('updater', message),
    error: (message: string) => logError('updater', message),
